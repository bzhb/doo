...
from doo.options_parsing import sort_options_and_arguments,process_options
        ...
        for i,line in enumerate(command_file):
            ...
            if line == '':
                ...
                if not separator:
                    ...
                    rest_of_line = keys
                    # By default we attribute a line number as key for the command
                    keys = str( len(data.commands)+1 )
                    if options.debug:
                        print("file_parsing: no «:» separator on line ",i+1,". Key is number ", keys)
                elif not keys:
                    # means no keys where given (line starts with ':')
                    keys = str( len(data.commands)+1 )
                    if options.debug:
                        print("file_parsing: no key given on line ",i+1,". Key is number ", keys)
                ...
                if options.add_numeric_key and not has_numeric_key:
                    ...
                    key = generate_unique_key(str( len(data.commands)+1 ))
                    keys_width += len(key) + 1 # the +1 is for the comma between keys
